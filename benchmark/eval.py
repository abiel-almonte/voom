...
def main():
    ...
    p.add_argument("--fp32", action="store_true", help="run eval in fp32 instead of fp16")
    args = p.parse_args()
    ...
    model = VOOMv2(**model_cfg).to(device=device, dtype=dtype)
    ...
    for i in range(len(ds)):
        rgb, K, gt_occ, gt_sem, _, _ = ds[i]
        rgb = rgb.unsqueeze(0).to(device=device, dtype=dtype)
        K = K.unsqueeze(0).to(device=device, dtype=dtype)
        ...
        with torch.no_grad():
            ...
            pred_alpha = pred_grid[:, 0:1].sigmoid()[0, 0]  # [gx, gy, gz]
            pred_occ_bin = pred_alpha > 0.26
            pred_class = pred_grid[:, 1:].argmax(dim=1)[0] + 1  # [gx, gy, gz]
            pred_class[~pred_occ_bin] = 0

            pred_occ = pred_occ_bin.float() * fmask
            gt_bin = ((gt_occ[0, 0] > 0.5).float()) * fmask
            ...
            if occ_mask.any():
                total_sem_correct += (
                    (pred_class[occ_mask] == gt_sem[occ_mask]).sum().item()
                )
